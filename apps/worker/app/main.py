...
app = FastAPI(title="CaseHunter AI Worker", version="0.8.0")
...
<div class="grid"><div class="card"><b>🔎 اكتشاف</b><span>Radar للأخبار والمصادر</span></div><div class="card"><b>📊 تحليل</b><span>تقييم القصص والمصادر</span></div><div class="card"><b>🎯 استهداف</b><span>تحديد الجمهور والصيغة</span></div><div class="card"><b>📝 تجهيز</b><span>Content Blueprint</span></div><div class="card"><b>✍️ سكربت</b><span>مسودة Short أصلية</span></div><div class="card"><b>🎬 إنتاج</b><span>خطة مشاهد وصوت وترجمة</span></div><div class="card"><b>🧩 Render</b><span>Render job جاهز للمزوّد</span></div><div class="card"><b>🚀 نشر</b><span>جاهز لربط منصات النشر</span></div><div class="card"><b>📈 Analytics</b><span>جاهز لتتبع النتائج</span></div><div class="card"><b>● API</b><span class="status">Online</span></div></div>
<p style="margin-top:28px">Health: <code>/health</code> · Pipeline: <code>/pipeline/run</code> · Script: <code>/pipeline/script</code> · Production: <code>/pipeline/production</code> · Render: <code>/pipeline/render</code></p>
</main></body></html>"""
...
def pipeline_render(request: RenderRequest) -> dict:
    try:
        return build_render_job(request.plan)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
